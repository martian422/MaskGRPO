...
#
# Adapted from huggingface/open-r1: https://github.com/huggingface/open-r1/blob/6a0cd5c8ad031fc75118a4ce7f42a4860c3d8dea/src/open_r1/utils/code_providers.py
    ...
    def execute_scripts(self, scripts: List[str], languages: List[str]) -> List[float]:
        ...
        if self.e2b_router_url is not None:
            ...
            for execution in executions:
                try:
                    m = re.search(r"__PASS_RATE__\s*([0-9]+(?:\.[0-9]+)?)", execution.logs['stdout'][0])
                    if m:
    ...
    async def _run_script(self, script: str, languages: List[str], semaphore: asyncio.Semaphore) -> float:
        ...
        async with semaphore:
            try:
                ...
                print("Error in `_run_script` from E2B sandbox")
                return 0.0
