...
if __name__ == '__main__':
    ...
    to_cot = False
    if to_cot:
        config.model.mmada.pretrained_model_path = "/data/models/mmada-8b-mixcot"
    ...
    if to_cot:
        questions_dir = '.eval/questions/cot_questions.txt'
    else:
        questions_dir = '.eval/questions/questions.txt'
    question_list = []
    with open(questions_dir) as data:
        for line in data:
            question_list.append(line)
    responses = ['' for i in range(len(question_list))]
    images = []
    for i, question in enumerate(tqdm(question_list)):
        batch_size = 1

        cot_prompt = "You should first think about the reasoning process in the mind and then provide the user with the answer. The reasoning process is enclosed within <think> </think> tags, i.e. <think> reasoning process here </think> answer here\n"
        if to_cot:
        ...
        global_steps = 128
        block_len = 8
        # output_ids = model.mmu_generate(input_ids, max_new_tokens=max_len, steps=global_steps, block_length=block_len) # note: here the steps means global steps. for each block, the step is steps/(max/block_length)
        output_ids = model.mmu_generate_mcat(input_ids, max_new_tokens=max_len, steps=global_steps, block_length=block_len, cfg_scale=1.0)
