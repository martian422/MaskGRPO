...
if __name__ == '__main__':
    ...
    mmu_image_root = '/nfs/mtr/code/MMaDA/generated_samples/mmu'
    file_list = os.listdir(mmu_image_root)
    responses = ['' for i in range(len(file_list))]
    images = []
    for i, file_name in enumerate(tqdm(file_list)):
        image_path = os.path.join(mmu_image_root, file_name)
        ...
        cot_prompt = ''
        question = cot_prompt + "Please describe this image in detail."
        ...
        output_ids = model.mmu_generate_mcat(input_ids, max_new_tokens=max_len, steps=global_steps, block_length=block_len)
