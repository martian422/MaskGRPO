...
def aggregate_results(directory="."):
    ...
    for setup, results in sorted(setups.items()):
        output_path = os.path.join(directory, "results.txt")
        with open(output_path, "w") as f:
            text = f"Setup: {setup}\n"
            text += f"Accuracy: {results['accuracy']:.2f}%\n"
            text += f"Avg Effective Tokens: {results['avg_effective_tokens']:.2f}\n"
            #save text
            f.write(text)
            
    print(f"Saved results for {setup} to {output_path}")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--mark", type=str, default="baseline")
    parser.add_argument("--task", type=str, default="gsm8k", choices=["gsm8k", "math500"])
    args = parser.parse_args()
    if args.task == "gsm8k":
        aggregate_results(directory=f"generated_samples/gsm8k/{args.mark}")
    elif args.task == "math":
        aggregate_results(directory=f"generated_samples/math500/{args.mark}")